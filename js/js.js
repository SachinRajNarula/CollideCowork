$(function () {
    $('.navbar-toggle').on('click', function (e) {
        ...
        $('.mobile-navbar-container').toggleClass('open')
    })
    ...
    var SignupViewModel = function () {
        ...
        self.submit = function () {
            ...
            for (var i = 1; i <= 30; i++) {
                ...
                //Make sure boolean or capitalised values are submitted as expected by server
                if (ko.unwrap(vm.coworker['Custom' + i]) === 'True' || ko.unwrap(vm.coworker['Custom' + i]) === true)
                    vm.coworker['Custom' + i]('true');
                else if (ko.unwrap(vm.coworker['Custom' + i]) === 'False' || ko.unwrap(vm.coworker['Custom' + i]) === false)
                    vm.coworker['Custom' + i](null);
            }
...
function BookingManager(settings) {
    ...
    var ViewModel = function () {
        ...
        this.includeZoomInvite = ko.observable(false);
    ...
    this.createBooking = function (settings) {
        ...
        if (coworker.Id == null && !guestBookings) {
            app.showQuestion(
                ...
                "You must have an account with Collide Coworking to make a booking. Would you like to sign in now?")
        ...
        if (coworker.Id == null && guestBookings) {
            app.showQuestion(
                "Are you already registered or have used any services at Collide Coworking?",
    ...
    this.showBooking = function (calEvent) {
        ...
        $.get(bookingUrl + '/' + vm.currentBookingId()).then(function (response) {
            ...
            vm.includeZoomInvite(response.Value.IncludeZoomInvite);
    ...
    function getBookingData() {
        return {
            Booking: {
                IncludeZoomInvite: vm.includeZoomInvite(),
                Notes: vm.notes(),
                DiscountCode: vm.discountCode(),
                Resource: {
